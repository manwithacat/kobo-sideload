...
the installer or `kobo-sideload dictionaries`) are listed in
`kobo_sideload/bundled/dictionaries.tsv`. Current rows come from the same
tar.gz files KOReader lists in `frontend/ui/data/dictionaries.lua`:
