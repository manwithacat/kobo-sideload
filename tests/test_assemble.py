...
class AssembleTests(unittest.TestCase):
    def test_payload_layout_drops_trigger_png_and_writes_nm_config(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("experimental:menu_main_15505_label:KOReader", nm)
            self.assertIn("menu_item:library:KOReader:", nm)
            self.assertIn("# experimental:menu_main_15505_enabled:0", nm)
